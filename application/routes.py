...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.get_password(password):
            ...
            flash("Sorry, Something gaalt","danger")     
...
def enrollment():
    ...
    if courseID:
        if Enrollment.objects(user_id=user_id, courseID=courseID):
            flash(f"Oops! you are already registered in course {courseTitle}","danger")
            return redirect(url_for("courses"))
        else:
            e = Enrollment(user_id=user_id,courseID=courseID)
            e.save()
...
@app.route("/user")
def user():
    #User(user_id=1, first_name="Yash", last_name="Gupta", email="y25.gupta@gmail.com", password="yash12345").save()
    #User(user_id=2, first_name="Doctor", last_name="Singh", email="doctorsingh@gmail.com", password="doctor12345").save()
